document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadData = async () => {
        try {
            ...
            if (error.message.includes('401')) { // Token expired
                localStorage.removeItem('jwt_token');
                checkLoginStatus();
            }
        }
    ...
    const renderCategories = () => { /* ... existing logic ... */ };
            ...
            allUsers.forEach(user => {
                ...
                if (user.username !== 'admin') {
                    li.querySelector('.delete-user-btn').addEventListener('click', async (e) => {
                        e.stopPropagation();
                        const confirmed = await showConfirmDialog('确认删除用户', `确定要删除用户 "${user.username}" 吗？`);
                        if(confirmed) {
    ...
    const populateUserForm = (user) => {
        ...
        userForm.querySelector('#user-form-password').value = '';
        ...
        allCategories.forEach(cat => {
            ...
            userFormCategories.innerHTML += `
                <div>
                    <input type="checkbox" id="cat-perm-${cat.id}" value="${cat.id}" ${isChecked ? 'checked' : ''}>
                    <label for="cat-perm-${cat.id}">${cat.name}</label>
                </div>
            `;
        });
    };
    ...
    const clearUserForm = () => {
        ...
        allCategories.forEach(cat => {
            userFormCategories.innerHTML += `
                <div>
                    <input type="checkbox" id="cat-perm-${cat.id}" value="${cat.id}">
                    <label for="cat-perm-${cat.id}">${cat.name}</label>
                </div>
            `;
        });
    ...
    userForm.addEventListener('submit', async (e) => {
        ...
        if (password) {
            userData.password = password; // The backend only uses this for new users, we need another endpoint for password changes
        }
